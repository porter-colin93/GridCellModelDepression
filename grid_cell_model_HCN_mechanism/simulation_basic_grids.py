...
state_record_e = np.random.randint(0,ei_net.Ne_y*ei_net.Ne_x,[ei_net.Ne_y*ei_net.Ne_x/10,])
state_record_i = np.random.randint(0,ei_net.Ni_y*ei_net.Ni_x,[ei_net.Ni_y*ei_net.Ni_x/10,])

state_record_ee = np.random.randint(0,ei_net.Ne_y*ei_net.Ne_x,size=(3000))
print(state_record_ee)
state_record_ii = np.random.randint(0,ei_net.Ni_y*ei_net.Ni_x,size=(500))
print(state_record_ii)
...
#stateMon_ee          = RecentStateMonitor(ei_net.E_pop, 'vm',     duration=options.stateMonDur*ms,   record = True, clock=simulationClock)
#stateMon_ii          = RecentStateMonitor(ei_net.I_pop, 'vm',     duration=options.stateMonDur*ms,   record = True, clock=simulationClock)
...
for dump_it in range(options.ndumps):
    ...
    # Plot a simulated LFP
    figure(figsize=figSize)
    plot(stateMon_e.times, np.sum(stateMon_e.values[:, :],axis=1) + np.sum(stateMon_i.values[:, :], axis=1))
    xlabel('Time (s)')
    ylabel('Summed $V_m$ (V)')
    xlim(x_lim)
    #axis('equal')
    tight_layout()
    savefig(output_fname + '_LFP.png')

    """
    # Plot a simulated LFP power spectrum

    from scipy import signal
    figure()

    LFP = np.sum(stateMon_e.values, axis=1) + np.sum(stateMon_i.values, axis=1)
    print(np.size(LFP))
    print(np.shape(LFP))
    fs = 10e3

    fxn, Pxx_den = signal.welch(LFP,fs)

    plt.semilogy(fxn, Pxx_den)
    plt.xlabel('Frequency (Hz)')
    plt.ylabel('PSD $(V**2/Hz)$')
    #axis('equal')
    tight_layout()
    plt.savefig(output_fname + '_LFP_fourier.png')
    """
