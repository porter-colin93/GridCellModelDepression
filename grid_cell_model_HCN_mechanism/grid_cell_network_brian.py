...
class BrianGridCellNetwork(GridCellNetwork):
    ...
    def __init__(self, neuronOpts, simulationOpts):
        ...
        self.eqs_e = Equations('''
            ...
            Im          = gL*(EL-vm) + g_ahp*(Eahp - vm) + Ispike + Isyn + Iext + IHCN : amp
            ...
            EL                                                                         : volt
            taum                                                                       : second
            IHCN        = g_h*s*(vm-Eh)                                                : amp
            ds/dt       = (s_inf - s)/(tau_h)                                          : 1
            s_inf       = 1/(1+exp((vm - v_half)/(8*mvolt)))                           : 1
            tau_h       = (F_tau*second*exp(0.033*(vm+75*mvolt)/(1*mvolt))) / (0.013*(1+exp(0.083*(vm+75*mvolt)/(1*mvolt))))   : second
            E_h                                                                        : volt
            ''',
        ...
        self.eqs_i = Equations('''
            dvm/dt      = 1/C*Im + (noise_sigma*xi/taum_mean**.5)           : volt
            Ispike      = gL*deltaT*exp((vm-Vt)/deltaT)                     : amp
            Im          = gL*(EL-vm)*(1+g_ad/gL) + Ispike + Isyn + Iext + IHCN                                          : amp
            ...
            IHCN        = g_h*s*(vm-Eh)                                     : amp
            ds/dt       = (s_inf - s)/tau_h                                : 1
            s_inf       = 1/(1+exp((vm - v_half)/(8*volt)))                        : 1
            tau_h       = second*(F_tau*exp(0.033*(vm+75*mvolt)/(1*mvolt)))/(0.013*(1+exp(0.083*(vm+75*mvolt)/(1*volt))))                           : second
            ''',
